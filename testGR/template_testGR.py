...
class TemplateGR(BaseGaussianNoise):
    ...
    def __init__(self, variable_params, data, low_frequency_cutoff,
        ...
        super(TemplateGR, self).__init__(
    ...
    def _loglr(self):
        ...
        for i in range(len(p['modes'])):
            l,m= p['modes'][i]
            f,hcross[l,m],hplus[l,m] = gen_signal_fre(p['chirpmass'],p['q'],p['distance'],p['inc'],p['phi0'],
                                   p['chi1'],p['chi2'],p['apx'],[p['modes'][i]],df=df)
